...
var playersRelations = relations(players, ({ one }) => ({
  room: one(rooms, {
  ...
  "domains": [
    {
      ...
      "words": [
        {
          "word": "Pizza",
          "similar": ["Pasta", "Burger", "Sandwich", "Taco", "Sushi"]
        },
        {
          "word": "Coffee",
          "similar": [
    ...
    },
    {
      ...
      "words": [
        {
          "word": "Dog",
          "similar": ["Cat", "Wolf", "Fox", "Puppy", "Husky"]
        },
        {
          "word": "Elephant",
          "similar": [
        ...
        {
          "word": "Eagle",
          "similar": ["Hawk", "Falcon", "Owl", "Vulture", "Condor"]
        },
        {
          "word": "Dolphin",
          "similar": ["Whale", "Shark", "Seal", "Porpoise", "Orca"]
        }
      ]
    },
    {
    ...
    },
    {
      ...
      "words": [
        {
          "word": "Beach",
          "similar": ["Ocean", "Coast", "Shore", "Island", "Resort"]
        },
        {
          "word": "Mountain",
          "similar": ["Hill", "Peak", "Summit", "Valley", "Cliff"]
        },
        {
          "word": "Hotel",
          "similar": ["Motel", "Resort", "Hostel", "Inn", "Lodge"]
        },
        {
          "word": "Airport",
          "similar": [
...
async function createApp(edgespark) {
  ...
  app.post("/api/public/rooms", async (c) => {
    ...
    let exists = await edgespark.db.select().from(db_schema_exports.rooms).where(eq(db_schema_exports.rooms.code, code));
    while (exists.length > 0) {
      code = generateCode();
      exists = await edgespark.db.select().from(db_schema_exports.rooms).where(eq(db_schema_exports.rooms.code, code));
    }
    const [room] = await edgespark.db.insert(db_schema_exports.rooms).values({
      code,
  ...
  app.post("/api/public/rooms/:code/join", async (c) => {
    ...
    const [room] = await edgespark.db.select().from(db_schema_exports.rooms).where(eq(db_schema_exports.rooms.code, code));
    if (!room) return c.json({ error: "Room not found" }, 404);
    if (room.status !== "lobby") return c.json({ error: "Game already started" }, 400);
    const players2 = await edgespark.db.select().from(db_schema_exports.players).where(eq(db_schema_exports.players.roomId, room.id));
    const isHost = players2.length === 0 ? 1 : 0;
    const [player] = await edgespark.db.insert(db_schema_exports.players).values({
      roomId: room.id,
      name,
      isHost,
      isAlive: 1
  ...
  app.get("/api/public/rooms/:code", async (c) => {
    ...
    if (!room) return c.json({ error: "Room not found" }, 404);
    const players2 = await edgespark.db.select().from(db_schema_exports.players).where(eq(db_schema_exports.players.roomId, room.id));
    return c.json({ room, players: players2 });
  });
  app.post("/api/public/rooms/:code/start", async (c) => {
    ...
    if (!room) return c.json({ error: "Room not found" }, 404);
    const players2 = await edgespark.db.select().from(db_schema_exports.players).where(eq(db_schema_exports.players.roomId, room.id));
    if (players2.length < 3) return c.json({ error: "Need at least 3 players" }, 400);
    const shuffled = [...players2].sort(() => Math.random() - 0.5);
    ...
    const specialWord = room.mode === "spy" ? wordObj.similar[Math.floor(Math.random() * wordObj.similar.length)] : "";
    const relatedCandidates = domain.words.filter((w) => w.word !== commonWord).map((w) => w.word);
    ...
    const updates = shuffled.map((p, i) => {
      ...
      return edgespark.db.update(db_schema_exports.players).set({ role, word, clue: null }).where(eq(db_schema_exports.players.id, p.id));
    });
    await Promise.all(updates);
    await edgespark.db.update(db_schema_exports.rooms).set({
      status: "selecting",
      currentTurnPlayerId: shuffled[0].id,
      options: JSON.stringify(options)
    }).where(eq(db_schema_exports.rooms.id, room.id));
  ...
  app.post("/api/public/rooms/:code/clue", async (c) => {
    ...
    const [room] = await edgespark.db.select().from(db_schema_exports.rooms).where(eq(db_schema_exports.rooms.code, code));
    if (!room) return c.json({ error: "Room not found" }, 404);
    await edgespark.db.update(db_schema_exports.players).set({ clue }).where(eq(db_schema_exports.players.id, playerId));
    const players2 = await edgespark.db.select().from(db_schema_exports.players).where(eq(db_schema_exports.players.roomId, room.id));
    const alivePlayers = players2.filter((p) => p.isAlive === 1);
    const allSubmitted = alivePlayers.every((p) => p.clue && p.clue.length > 0 || p.id === playerId);
    if (allSubmitted) {
      await edgespark.db.update(db_schema_exports.rooms).set({ status: "voting" }).where(eq(db_schema_exports.rooms.id, room.id));
    }
    return c.json({ success: true });
  });
  app.post("/api/public/rooms/:code/vote", async (c) => {
    ...
    const [target] = await edgespark.db.select().from(db_schema_exports.players).where(eq(db_schema_exports.players.id, targetPlayerId));
    if (target) {
      await edgespark.db.update(db_schema_exports.players).set({ votes: (target.votes || 0) + 1 }).where(eq(db_schema_exports.players.id, targetPlayerId));
    }
    return c.json({ success: true });
  });
  app.post("/api/public/rooms/:code/eliminate", async (c) => {
    ...
    if (toEliminate) {
      await edgespark.db.update(db_schema_exports.players).set({ isAlive: 0 }).where(eq(db_schema_exports.players.id, toEliminate.id));
      if (toEliminate.role === "infiltrator" || toEliminate.role === "spy") {
        await edgespark.db.update(db_schema_exports.rooms).set({ status: "ended" }).where(eq(db_schema_exports.rooms.id, room.id));
        return c.json({ eliminated: toEliminate, winner: "citizens" });
      }
      const alive = players2.filter((p) => p.id !== toEliminate.id && p.isAlive === 1);
      if (alive.length <= 2) {
        await edgespark.db.update(db_schema_exports.rooms).set({ status: "ended" }).where(eq(db_schema_exports.rooms.id, room.id));
        return c.json({ eliminated: toEliminate, winner: "special" });
      }
    }
    await edgespark.db.update(db_schema_exports.players).set({ votes: 0 }).where(eq(db_schema_exports.players.roomId, room.id));
